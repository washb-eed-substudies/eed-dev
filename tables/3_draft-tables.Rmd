...
bg_tables <- bg_tables %>% 
  mutate(`Outcome Subdomain` = case_match(`Outcome Subdomain`,
         ...
         across(contains("val"), 
                ~ round(., 2))) %>% 
  rename(`FDR Corrected P-value` = corrected.Pval_adj)
...
flex_format <- function(ft, len_col_keys = 9){
  ft %>% 
...
bg_hr_tab <- flextable(bg_hr_tbl, 
  ...
  flex_format(1+9) %>% 
  set_caption("EED and WHO Motor Milestones in Bangladesh (Hazard Ratios, Year 1)", style = "Table Caption",
              autonum = autonum)
```
...
k_hr_tab <- flextable(k_hr_tbl, 
                      col_keys = c("Motor Milestone", 
                                   append(keep_cols, "Adj. HR (95% CI)", 6))) %>% 
  flex_format(1+9) %>% 
  set_caption("EED and WHO Motor Milestones in Kenya (Hazard Ratios, Year 1)", style = "Table Caption",
              autonum = autonum)
```

# Full Tables
```{r}
bg_tables_tab <- bg_tables %>% 
  as_grouped_data(groups= "Outcome Domain") %>% 
  flextable(col_keys = append(keep_cols, "Adj. Diff (95% CI)", 6)) %>% 
  flex_format(1+8) %>% 
  ...
  width(j = "Outcome Domain", 
        width = 2.9) %>% 
  # fit_to_width(max_width=10.75) %>% 
  set_caption("EED and Child Development in Bangladesh", style = "Table Caption",
              autonum = autonum)
```

```{r}
k_tables_tab <- k_tables %>% 
  as_grouped_data(groups= "Outcome Domain") %>% 
  flextable(col_keys = append(keep_cols, "Adj. Diff (95% CI)", 6)) %>%
  flex_format(1+8) %>% 
  merge_v("Outcome Domain") %>% 
  width(j = "Outcome Domain", 
        width = 2.9) %>% 
  set_caption("EED and Child Development in Kenya", style = "Table Caption",
              autonum = autonum)
```
...
```{r}
# ordered for supplemental tables
save_as_docx(bg_tables_tab, 
             bg_hr_tab,
             ...
             path = here::here('tables/eed-dev-supptables.docx'), 
             pr_section = prop_section(
               page_size = page_size(orient = 'landscape'), 
               page_margins = page_mar(bottom = 1, 
                                       top = 1, 
...
# Add worksheets
addWorksheet(wb, "Bangladesh WHO")
addWorksheet(wb, "Kenya WHO")
addWorksheet(wb, "Bangladesh EASQ")
addWorksheet(wb, "Kenya EASQ")
addWorksheet(wb, "Bangladesh HR")
addWorksheet(wb, "Kenya HR")

writeData(wb, "Bangladesh WHO", bg_who_tbl)
writeData(wb, "Kenya WHO", k_who_tbl)
writeData(wb, "Bangladesh EASQ", bg_easq_tbl)
writeData(wb, "Kenya EASQ", k_easq_tbl)
writeData(wb, "Bangladesh HR", bg_hr_tbl)
writeData(wb, "Kenya HR", k_hr_tbl)


# Save the workbook
saveWorkbook(wb, here::here("tables/all_tables.xlsx"), overwrite = TRUE)
