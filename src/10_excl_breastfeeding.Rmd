  ...
  suppressMessages({
    ...
    for(i in Xvars){
      for(j in Yvars){
        ...
        print(j)
        res_adj = NULL
        try(res_adj <- fit_RE_gam(d=data, X=Xvars, Y=Yvars, W=covariates, forcedW = forced_cov))
        if(is.null(res_adj)){
          try(res_adj <- fit_RE_gam(d=data, X=Xvars, Y=Yvars, W=covariates, forcedW = forced_cov, vim=FALSE))
        }
